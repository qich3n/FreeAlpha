/* ── FreeAlpha frontend (Cloudflare Pages edition) ───────────────── */
...
async function loadChain() {
  ...
  try {
    ...
    // Compute Greeks client-side
    computeChainGreeks(data.chain, data.spot, RISK_FREE_RATE);

    // Compute aggregate exposure
    data.exposure = aggregateExposure(data.chain, data.spot);
...
function aggregateExposure(chain, spot) {
  const byStrike = {};
  let totalGex = 0, totalDex = 0, totalVex = 0, totalCex = 0;

  for (const r of chain) {
    const k = r.strike;
    if (!byStrike[k]) byStrike[k] = { strike: k, gex: 0, dex: 0, vex: 0, cex: 0 };

    const oi = r.openInterest;
    const sign = r.type === "call" ? 1 : -1;

    const g = oi * r.gamma * CONTRACT_MULTIPLIER * spot * sign;
    const d = oi * r.delta * CONTRACT_MULTIPLIER * sign;
    const v = oi * r.vega * CONTRACT_MULTIPLIER * sign;
    const c = oi * r.charm * CONTRACT_MULTIPLIER * sign;
  ...
  const profile = strikes.map((k) => ({
    strike: k,
    gex: Math.round(byStrike[k].gex * 100) / 100,
    dex: Math.round(byStrike[k].dex * 100) / 100,
    vex: Math.round(byStrike[k].vex * 100) / 100,
    cex: Math.round(byStrike[k].cex * 100) / 100,
  }));

  return {
    total_gex: Math.round(totalGex * 100) / 100,
    total_dex: Math.round(totalDex * 100) / 100,
    total_vex: Math.round(totalVex * 100) / 100,
    total_cex: Math.round(totalCex * 100) / 100,
    profile,
  };
}
...
function renderCharts(data) {
  ...
  createBarChart("gexChart", "gex", labels, profile.map((p) => p.gex), "GEX");
  createBarChart("dexChart", "dex", labels, profile.map((p) => p.dex), "DEX");
  createBarChart("vexChart", "vex", labels, profile.map((p) => p.vex), "VEX");
  createBarChart("cexChart", "cex", labels, profile.map((p) => p.cex), "CEX");
}
...
function createBarChart(canvasId, key, labels, values, label) {
  ...
  charts[key] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: { label: (ctx) => `${label}: ${fmtLarge(ctx.raw)}` },
        },
      },
      scales: {
        x: {
